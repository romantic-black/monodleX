...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        # basic configuration
        self.root_dir = cfg.get('root_dir', '/mnt/e/Dataset/kitti')
        self.split = split
        self.num_classes = 3
        self.max_objs = 50
        ...
        self.split_file = os.path.join('/home/a/monodleX/data/KITTI', 'ImageSets', self.split + '.txt')
    ...
    def get_road_plane(self, idx):
        plane_file =  os.path.join(self.road_dir, '%06d.txt' % idx)
    ...
    def put_to_ground(self, points, road):
        a, b, c, d = road
        y = (a * points[:, 0] + c * points[:, 2] + d) / -b
        points[:, 1] = y
        return points
    ...
    def __getitem__(self, item):
        ...
        road = self.get_road_plane(index)
        ...
        if self.bbox2d_type == 'proj':  # False
            for object in objects:
                object.box2d_proj = np.array(calib.corners3d_to_img_boxes(object.generate_corners3d()[None, :])[0][0],
                                             dtype=np.float32)
        ...
        size_2d = np.zeros((self.max_objs, 2), dtype=np.float32)  # 真值居然是这种形式
        ...
        for i in range(object_num):
            ...
            # add affine transformation for 2d boxes.   # 同样被降采样
            bbox_2d[:2] = affine_transform(bbox_2d[:2], trans)
            bbox_2d[2:] = affine_transform(bbox_2d[2:], trans)
            # modify the 2d bbox according to pre-compute downsample ratio
            bbox_2d[:] /= self.downsample

            # process 3d bbox & get 3d center   # center_3d 是 3d 中心在图像的投影，顺便还被降采样了
            ...
            mean_size = self.cls_mean_size[self.cls2id[objects[i].cls_type]]    # 没设置
